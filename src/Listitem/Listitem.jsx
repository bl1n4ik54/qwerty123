...
export const TaskInput = () => {
	...
	const createElemToDo = () => {
		if (task !== "") {
			let copy = [...tasks, { id: randomId(16), text: task, isEdit: false, isChecked: false }]
			setTasks(copy)
			setTask('')
			func(copy)
		}
	...
	const saveTask = (id, text) => {
		let copy = tasks.map((task) => task.id == id ? { ...task, text } : task)
		setTasks(copy)
		func(copy)
	}
				...
				{tasks.map(task => (
					<>
						<li key={task.id}>
							...
							<button className={s.iconChange} onClick={() => toggler(task.id)}>
							...
							<button className={s.iconDelite} onClick={() => deleteTask(task.id)}>
								<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
